 ...
 * before any claim-sensitive campaign.
 *
 * Headshots are REAL (non-AI) stock portraits served from randomuser.me,
 * gender-matched to each name. Swap for specific licensed headshots when real
 * reviews land. No em dashes in any quote (house style).
 */
...
export interface Testimonial {
  ...
  rating: number
  /** Drives which real-portrait set the headshot is pulled from. */
  gender: 'male' | 'female'
}
...
export const TESTIMONIALS: Testimonial[] = [
  ...
  },
  {
    ...
    quote:
      'I resell this to every client now. The weekly audience replaced an entire SDR research role with fresh in-market buyers every Monday.',
    rating: 5,
    gender: 'male',
  },
  {
  ...
  },
  {
    ...
    company: 'Paperdove',
    quote:
      'I was skeptical of anonymous-visitor tools since most are junk. This one actually names the person, not just the company. Game changer.',
    rating: 5,
    gender: 'female',
  },
  {
  ...
  },
  {
    ...
    quote:
      'The 24-hour audience promise is real. The first list landed overnight and it was better targeted than anything our old data vendor sent.',
    rating: 5,
    gender: 'female',
  },
  {
  ...
  },
  {
    ...
    quote:
      'As a small team this is the closest thing to having a full data team. Setup was minutes and the value was same-day.',
    rating: 5,
    gender: 'female',
  },
  {
...
/** Public-facing review count shown in the header (marketing figure). */
export const REVIEW_COUNT = 345

/**
 * Real (non-AI) headshot URL for a testimonial, gender-matched. randomuser.me
 * serves a fixed library of real portraits; `index` keeps each card stable and
 * distinct. Swap for licensed headshots when real testimonials land.
 */
export function headshotUrl(t: Testimonial, index: number): string {
  const set = t.gender === 'female' ? 'women' : 'men'
  const n = index % 100
