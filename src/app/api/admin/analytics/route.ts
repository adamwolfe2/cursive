...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      supabase.from('workspaces').select('id', { count: 'estimated', head: true }),
      supabase.from('workspaces').select('id', { count: 'estimated', head: true })
        .eq('is_active', true),
      supabase.from('partners').select('id', { count: 'estimated', head: true })
        .eq('is_active', true),
      ...
      supabase.from('leads').select('id', { count: 'estimated', head: true })
        ...
        .gte('created_at', startDate.toISOString()),
      supabase.from('email_sends').select('id', { count: 'estimated', head: true })
