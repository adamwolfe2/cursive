-- Intelligence layer: idempotent recovery migration
-- Adds new columns on leads + creates enrichment_costs table
-- Originally archived from 20260226_intelligence_layer.sql due to migration order conflicts
...
DROP POLICY IF EXISTS "workspace_isolation" ON enrichment_costs;
...
DROP POLICY IF EXISTS "service_role_bypass" ON enrichment_costs;
CREATE POLICY "service_role_bypass" ON enrichment_costs
  FOR ALL TO service_role USING (true) WITH CHECK (true);

-- Indexes for admin margin queries
