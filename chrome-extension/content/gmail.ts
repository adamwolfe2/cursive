 ...
 *
 * Injects into Gmail to verify email addresses and provide
 * inline enrichment cards for contacts.
 */
...
interface GmailEnrichmentResult {
  first_name: string
  last_name: string
  title: string
  company: string
...
interface GmailCacheEntry {
  ...
  enrichment?: GmailEnrichmentResult
  timestamp: number
}
...
function gmailCacheKey(email: string): string {
  return `cursive_gmail_${email.toLowerCase().trim()}`
}

async function getGmailCache(email: string): Promise<GmailCacheEntry | null> {
  return new Promise((resolve) => {
    const key = gmailCacheKey(email)
    chrome.storage.local.get(key, (result) => {
      const entry = result[key] as GmailCacheEntry | undefined
      if (!entry) {
        resolve(null)
        return
      }
...
async function setGmailCache(email: string, update: Partial<GmailCacheEntry>): Promise<void> {
  return new Promise((resolve) => {
    const key = gmailCacheKey(email)
    chrome.storage.local.get(key, (result) => {
      const existing = (result[key] as GmailCacheEntry) || { timestamp: Date.now() }
      const merged: GmailCacheEntry = {
        ...existing,
        ...update,
        timestamp: Date.now(),
      }
...
function extractEmailFromReadView(container: Element): string | null {
  ...
  if (emailSpan) {
    ...
    if (email && EMAIL_REGEX.test(email)) return email.toLowerCase().trim()
  }
  ...
  if (hovercardEl) {
    ...
    if (hcid && EMAIL_REGEX.test(hcid)) return hcid.toLowerCase().trim()
  }
...
function extractEmailFromComposeView(): string | null {
...
function createBadge(email: string, status: 'valid' | 'catch-all' | 'invalid' | 'checking'): HTMLElement {
...
function createEnrichmentCard(data: GmailEnrichmentResult, anchor: HTMLElement): void {
  ...
  const closeBtn = card.querySelector('.cursive-card-close') as HTMLElement
  ...
  const outsideHandler = (e: MouseEvent) => {
    if (!card.contains(e.target as Node) && e.target !== anchor) {
      removeEnrichmentCard()
...
function createLoadingCard(anchor: HTMLElement): void {
  removeEnrichmentCard()
...
function removeEnrichmentCard(): void {
...
async function verifyAndBadge(email: string, anchorEl: Element): Promise<void> {
  if (anchorEl.querySelector(`[data-cursive-email="${CSS.escape(email)}"]`)) return
  ...
  chrome.runtime.sendMessage(
    { type: 'VERIFY_EMAIL', data: { email } },
    async (response) => {
      if (chrome.runtime.lastError) return

      if (response?.data) {
        const verify = response.data as VerifyResponse
...
async function handleBadgeClick(email: string, badge: HTMLElement): Promise<void> {
  ...
  chrome.runtime.sendMessage(
    ...
    async (response) => {
      ...
      if (response?.data) {
        const enrichment = response.data as GmailEnrichmentResult
...
function scanReadEmails(): void {
  ...
  const hovercardEls = document.querySelectorAll(
    `[data-hovercard-id]:not([${PROCESSED_ATTR}])`
  )
  hovercardEls.forEach((el) => {
    const hcid = el.getAttribute('data-hovercard-id')
    if (!hcid || !EMAIL_REGEX.test(hcid)) return
...
function scanComposeEmails(): void {
  ...
  composeContainers.forEach((container) => {
    if (container.getAttribute(PROCESSED_ATTR) === email) return
